...
const setupServer = require('./build/setup-dev-server');
const app = express()
...
let appPath = [path.join(clientConfig.output.path, 'msite.html')]//client ssr 页面
let outputPath = [path.join(serverConfig.output.path, 'server/msite/server-bundle.js')]
...
if (isProd) {
  ...
  renderer = createRenderer(fs.readFileSync(resolve('./www/server/msite/server-bundle.js'), 'utf-8'))

  indexHTML = parseIndex(fs.readFileSync(resolve('./www/msite.html'), 'utf-8'))

} else {
  // in development: setup the dev server with watch and hot-reload,
  // and update renderer / index HTML on file change.
  setupServer(appPath,outputPath)(app, {
...
app.get('*', (req, res) => {
  if(isProd){
    renderer = createRenderer(fs.readFileSync(resolve('./www/server'+req.url+'/server-bundle.js'), 'utf-8'))
    indexHTML = parseIndex(fs.readFileSync(resolve('./www'+req.url+'.html'), 'utf-8'))
  }else{
    appPath[0] = path.join(clientConfig.output.path, req.url.replace(/^\//,'')+'.html')//client ssr 页面
    outputPath[0] = path.join(serverConfig.output.path, 'server'+req.url+'/server-bundle.js')
